...
module param
  ...
  real*8, parameter :: alpha_gb_m2(1:maxzat0) = (/&
      ...
      0d0, 0d0, 316.5d0, 307.9d0, 297.7d0, 288.5d0,&
      279.6d0, 271.3d0, 263.9d0, 255.1d0, 247.2d0, 240.2d0,&
      233.4d0, 227d0, 0d0, 0d0, 180.1d0, 132.9d0,&
      111.1d0, 97.21d0, 88.26d0, 81.29d0, 75.64d0, 70.91d0,&
      0d0, 0d0, 186.4d0, 108.3d0, 75.8d0, 64.02d0,&
      0d0, 0d0, 0d0, 0d0, 0d0, 0d0,&
      0d0, 0d0, 0d0, 0d0, 0d0, 0d0,&
      0d0, 0d0, 0d0, 0d0, 0d0, 0d0,&
      0d0, 0d0, 0d0, 0d0, 0d0, 0d0,&
  ...
  ! Neutral-calibrated charge factors for the polarizability from confined-ion
  ! Sternheimer (uncoupled coupled-perturbed KS) solves on the SAME box-confined
  ! ld1.x ions used for the density references (Stage 2C-rigorous, generated by
  ! tools/wfc_generator/batch_sternheimer.py at rmax=3.6*R99). rstern_pq(Z) =
  ! alpha_stern(Z,q)/alpha_stern(Z,0); the charge-aware free-ion polarizability
  ! is alpha_free(Q) = alpha_free^CRC(Z) * rstern interpolated in N. The
  ! uncoupled absolute overestimate cancels in the ratio (validated vs the
  ! Gould-Bucko TDDFT trend: cations near-exact). Rows 1-3 (Z<=18) computed;
  ! Z>18 default to 1.0. Diffuse alkali/alkaline/noble anion ratios (never used
  ! -- those elements are not anions) are capped at 8. Provenance file:
  ! dat/xdm_ion_alpha_sternheimer.dat
real*8, parameter :: rstern_p1(1:maxzat0) = (/&
      1d0, 0.20386d0, 0.00124d0, 0.32929d0, 0.53242d0, 0.50393d0,&
      0.48759d0, 0.47762d0, 0.47223d0, 0.4704d0, 0.00674d0, 0.30444d0,&
      0.40125d0, 0.47427d0, 0.52591d0, 0.56447d0, 0.5946d0, 0.6189d0,&
      0.02463d0, 0.32089d0, 0.46882d0, 0.48329d0, 0.49156d0, 0.50904d0,&
      0.35721d0, 0.49978d0, 0.50004d0, 0.49973d0, 0.57443d0, 0.35568d0,&
      0.35657d0, 0.44547d0, 0.50927d0, 0.55674d0, 0.59374d0, 0.62356d0,&
      0.03533d0, 0.32567d0, 0.48113d0, 0.51496d0, 0.5844d0, 0.45495d0,&
      0.39233d0, 0.67146d0, 0.48192d0, 1.0223d0, 0.17209d0, 0.37896d0,&
      0.4292d0, 0.5031d0, 0.55856d0, 0.6005d0, 0.63347d0, 0.66028d0,&
      0.04647d0, 0.33621d0, 0.53298d0, 0.48034d0, 0.40566d0, 8d0,&
      8d0, 0.45048d0, 0.45d0, 0.51299d0, 0.41019d0, 0.44769d0,&
      0.4468d0, 0.44595d0, 0.4451d0, 0.44447d0, 0.47194d0, 0.51492d0,&
      0.54016d0, 0.55528d0, 0.56415d0, 0.56932d0, 0.57173d0, 0.72631d0,&
      0.52101d0, 0.40462d0, 0.46034d0, 0.52082d0, 0.57236d0, 0.61244d0,&
      0.64417d0, 0.67009d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0 /)
  ! rstern_p2: +2 cation Sternheimer ratios alpha(Z,+2)/alpha(Z,0). Generated 2026-08 by
  ! batch_sternheimer.py (q=+2 added; /usr/bin/ld1.x). Filled Z<=54; 1d0 = untabulated (>54).
  ! +2 cations are bound/closed-shell so the uncoupled Sternheimer is well-behaved (unlike -2
  ! anions which diverge). e.g. Mg(12)=0.0046 (alpha 0.57 a0^3), Ca(20)=0.0179 (alpha 5.2).
  real*8, parameter :: rstern_p2(1:maxzat0) = (/&
      1d0, 1d0, 0.00036137d0, 0.00074836d0, 0.2199d0, 0.40024d0,&
      0.36623d0, 0.34033d0, 0.32071d0, 0.30638d0, 0.0042411d0, 0.0046094d0,&
      0.15139d0, 0.29024d0, 0.34081d0, 0.38025d0, 0.41223d0, 0.43883d0,&
      0.017782d0, 0.017909d0, 0.18417d0, 0.29896d0, 0.30627d0, 0.31871d0,&
      0.31323d0, 0.20466d0, 0.31521d0, 0.31528d0, 0.36255d0, 0.31441d0,&
      0.14742d0, 0.2671d0, 0.32203d0, 0.36744d0, 0.40548d0, 0.43784d0,&
      0.025392d0, 0.025311d0, 0.19208d0, 0.32978d0, 0.37957d0, 0.41343d0,&
      0.24084d0, 0.29742d0, 0.45194d0, 0.64286d0, 0.30216d0, 0.076212d0,&
      0.19005d0, 0.32327d0, 0.37368d0, 0.41586d0, 0.45126d0, 0.48154d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
  ...
  real*8, parameter :: rstern_m1(1:maxzat0) = (/&
      ...
      8d0, 4.6337d0, 3.2758d0, 2.3562d0, 2.4512d0, 8d0,&
      8d0, 2.5904d0, 3.1768d0, 6.9813d0, 3.9125d0, 1d0,&
      1d0, 3.8066d0, 1.8777d0, 4.0481d0, 1d0, 8d0,&
      8d0, 7.1338d0, 3.5659d0, 2.3647d0, 2.4091d0, 8d0,&
      3.733d0, 8d0, 1.4243d0, 2.5418d0, 8d0, 1d0,&
      1d0, 1d0, 0.30932d0, 3.3275d0, 7.4729d0, 7.0992d0,&
      8d0, 5.6126d0, 3.0494d0, 2.0861d0, 2.1418d0, 8d0,&
      8d0, 8d0, 8d0, 2.7341d0, 4.6839d0, 8d0,&
      8d0, 8d0, 6.7821d0, 1d0, 8d0, 8d0,&
      8d0, 8d0, 8d0, 1d0, 7.9981d0, 1d0,&
      3.0826d0, 2.7978d0, 4.0338d0, 4.2193d0, 1d0, 1d0,&
      5.531d0, 8d0, 8d0, 5.7904d0, 2.9485d0, 2.0599d0,&
      2.0297d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&
      1d0, 1d0, 1d0, 1d0, 1d0, 1d0,&

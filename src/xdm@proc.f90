...
999 continue
  ...
  !> raw Watson-confined alpha aws_m{1,2,3}, which stay finite and match Tessman in-crystal for
  ...
  !> of the neutral). q clamped to [-3,+2]; aws=0 (over-fill) => neutral fallback.
  function ion_alpha_sternws(iz,q) result(a)
    use param, only: alpha_free, rstern_p1, rstern_p2, aws_m1, aws_m2, aws_m3, maxzat0
    integer, intent(in) :: iz
    real*8, intent(in) :: q
    real*8 :: a, qc, ratio, f, aq, e1, e2
    a = 0d0
    if (iz < 1 .or. iz > maxzat0) return
    if (q >= 0d0) then
       ! cations/neutral are bound => reuse the plain confined-Sternheimer ratios
       ! normalized to the accurate free-atom alpha (rstern_p1/p2, validated).
       qc = min(q,2d0)
       if (qc >= 1d0) then
          f = qc - 1d0; ratio = (1d0-f)*rstern_p1(iz) + f*rstern_p2(iz)
       else
       ...
       ! anions: use the RAW Watson-confined alpha (a0^3, aws_m{1,2,3}) directly.
       ! It already equals the in-crystal polarizability (matches Tessman: O2- 1.5,
       ! S2- 4.8 A^3). The neutral-based renormalization used for cations is NOT
       ! applied -- the LDA-uncoupled error is not transferable across shell-filling,
       ! and applying it pulls the deep anion below experiment. Interpolate in |q|:
       ! q=0 -> alpha_free, q=-1 -> aws_m1, -2 -> aws_m2, -3 -> aws_m3. A zero datum
       ! (unphysical over-fill, e.g. F2-) falls back to neutral free-atom alpha.
       aq = min(-q,3d0)
       if (aq <= 1d0) then
          e1 = alpha_free(iz); e2 = aws_m1(iz); f = aq
       else if (aq <= 2d0) then
          e1 = aws_m1(iz); e2 = aws_m2(iz); f = aq - 1d0
       else
          e1 = aws_m2(iz); e2 = aws_m3(iz); f = aq - 2d0
       end if
       if (e1 <= 0d0 .or. e2 <= 0d0) then
          a = alpha_free(iz)                    ! no Watson datum => neutral fallback
       else
          a = (1d0-f)*e1 + f*e2
       end if
    end if
